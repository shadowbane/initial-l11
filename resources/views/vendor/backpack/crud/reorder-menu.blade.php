...
@section('header')
    <section class="header-operation container-fluid animated fadeIn d-flex align-items-baseline d-print-none"
        ...
        <p class="ms-2 ml-2 mb-0"
           bp-section="page-subheading">{!! $crud->getSubheading() ?? trans('backpack::crud.reorder').' '.$crud->entity_name_plural !!}</p>
...
@section('content')
    ...
    function tree_element($entry, $key, $all_entries, $crud) {
        if (!isset($entry->tree_element_shown)) {
            ...
            echo '<div><span class="disclose"></span>'.object_get(
                    $entry,
                    $crud->get('reorder.label')
                ).'<span class="float-end font-sm text-muted">'.$entry->grouping.'</span></div>';
    ...
    <div class="row mt-4">
        <div class="{{ $crud->getReorderContentClass() }}">
            ...
            <div class="mt-3">
                <button id="toArray" class="btn btn-success text-light" data-style="zoom-in"><i
