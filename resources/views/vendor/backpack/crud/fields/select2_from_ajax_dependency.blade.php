...
@php
    ...
    $function_name = 'bpFieldInitSelect2FromAjaxElement'.\Illuminate\Support\Str::random(4);
@endphp
...
<select
    name="{{ $field['name'] }}"
    style="width: 100%"
    data-init-function="{{ $function_name }}"
    ...
    @if ($old_value)
        @php
            if(!is_object($old_value)) {
                $item = isset($field['edit_key'])
                    ? $connected_entity->where($field['edit_key'], $old_value)->first()
                    : $connected_entity->find($old_value);
            } else {
                $item = $old_value;
            }
        ...
        @if ($item)
            ...
            <option value="{{ isset($field['edit_key']) ? $item->{$field['edit_key']} : $item->getKey() }}" selected>
...
@push('crud_fields_scripts')
    <script>
        function {{ $function_name }}(element) {
            ...
            var select2AjaxDependencyFetchSelectedEntry = function (element) {
                ...
                ajax: {
                    ...
                    data: function (params) {
                        if ($includeAllFormFields) {
                            ...
                            return {
                                {{ $field['parent'] }}: $("[name='{{ $field['parent'] }}']").val(),
                                q: params.term, // search term
                                page: params.page, // pagination
                            };
                        }
            ...
            if (typeof $selectedOptions !== typeof undefined &&
                ...
                select2AjaxDependencyFetchSelectedEntry(element).then(result => {
