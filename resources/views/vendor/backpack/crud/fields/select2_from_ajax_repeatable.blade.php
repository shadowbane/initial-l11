...
@php
    ...
    $is_custom_date_field = false;
    if (isset($field['is_custom_date_field'])) {
        $is_custom_date_field = $field['is_custom_date_field'];
    }
@endphp
...
<select
    name="{{ $field['name'] }}"
    data-init-function="bpFieldInitSelect2FromAjaxElementRepeatable"
    ...
    data-is-custom-date-field="{{ $is_custom_date_field }}"
...
>
    ...
    @if ($old_value)
        ...
        @if ($item)
            ...
            @if($is_custom_date_field && $item instanceof \Carbon\Carbon)
                <option value="{{ $item->format('Y-m-d') }}" selected>
                    {{ $item->format('Y-m-d') }}
                </option>
            @else
...
@push('crud_fields_scripts')
    @loadOnce('bpFieldInitSelect2FromAjaxElementRepeatable')
    <script>
        function bpFieldInitSelect2FromAjaxElementRepeatable(element) {
                ...
                ajax: {
                    ...
                    data: function (params) {
                        if ($includeAllFormFields) {
                            return {
                                rowNumber: $(this).attr('data-row-number'),
